...
public class PushSingleServiceImpl implements Service{
	private static final Logger logger = Logger.getLogger(PushSingleServiceImpl.class);
	@Override
	public Object sevice(String param) {
		...
		logger.debug("push目标========="+pushMsg.getTarget()+",消息类型=========="+pushMsg.getMsgType()+",消息:"+pushMsg.getMsg());
		
		//获取目标信息
		 MerchantUser userInfo =handlerUserInfo(pushMsg.getTarget(),pushMsg.getMerchantCode());
		if(userInfo==null){
			logger.debug("push目标========="+pushMsg.getTarget()+",消息类型=========="+pushMsg.getMsgType()+",消息:"+pushMsg.getMsg()+",异常:未找到对应的员工信息!");
		}else{
	...
	private MerchantUser handlerUserInfo(String target,String merchantCode) {
		...
		String msg = BapUtil.generateRequestMessage(param,"queryAppUserInfo");
		logger.info("handlerUserInfo---msg ===="+msg);
		...
		logger.info("handlerUserInfo---url ===="+url);
		...
		if(!StringUtils.isEmpty(userInfo.getDeviceToken())){
	        ...
	        //String sound="newOrder.caf";
			  ...
			  List<String> tokens = new ArrayList<String>();
		        ...
		        //上生产前，证书改为生产证书
                // String certificatePath = this.getClass().getResource("/").getPath()+"developeMent0808.p12";
		        String certificatePath = this.getClass().getResource("/").getPath()+"sweepCodePayDevelopment.p12";
		        ...
		        try{
		            ...
		            String title=pushMsg.getMsgTitle();
		            String url=pushMsg.getMsgUrl();
...
//		            int successful = successfulNotifications.size();
	...
	public static void main(String[] args) throws Exception 
	 {
        String deviceToken = "e0670d5418c1933f899094b210645831e465015a835d7960d2adb16bc7b27152";
        String alert = "测试!";//push的内容
        int badge = 999;//图标小红圈的数值
        String sound = "default";//铃音

        List<String> tokens = new ArrayList<String>();
        tokens.add(deviceToken);
        String certificatePath = "C:/Users/Administrator/Desktop/sweepCodePayDevelopment.p12";
        String certificatePassword = "123456";//此处注意导出的证书密码不能为空因为空密码会报错
        boolean sendCount = true;
        ...
        try
        {
           ...
           // payLoad.addCustomAlertBody("噢嘿嘿~捏嘻嘻~哇咔咔咔~");
            ...
            payLoad.addCustomDictionary("title", "标题");
