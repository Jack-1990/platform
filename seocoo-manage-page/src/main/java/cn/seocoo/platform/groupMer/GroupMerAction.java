package cn.seocoo.platform.groupMer;
...
/**
 * 代理商门店      根据  merchantAction而来
 */
public class GroupMerAction extends BaseAction{
...
//		Integer beginRow = (Integer.parseInt(pageIndex)) * Constant.PAGESIZE_TEN;
		...
		if(!StringUtil.isEmpty(groupCode)){
			...
			List<GroupMerchantPay> payList=groupMerchantPayService.queryGroupMerchantPayList(pay);
			if(payList !=null && payList.size()>0){
				merchantCode = payList.get(0).getMerchantCode();
			}
		}
		...
		if(!StringUtil.isEmpty(merchantCode)){
			Map map = new HashMap();
			map.put("outMchntId", merchantCode);
			...
			merchantPage= merchantService.queryMerchantPage(map);
			// 获取支付通道中该商家绑定了那些通道
    ...
    public void saveMerchant() throws IOException {
        ...
        try {
    		...
    		//新增的是代理商商户，写入关系表
            if(!StringUtil.isEmpty(merchant.getGroupCode())){
            	GroupMerchantPay pay=new GroupMerchantPay();
            	pay.setGroupCode(merchant.getGroupCode());
            	List<GroupMerchantPay>  payList=groupMerchantPayService.queryGroupMerchantPayList(pay);
            	if(payList !=null && payList.size()>0){
            		pay.setMerchantCode(merchant.getOutMchntId());
            		groupMerchantPayService.updateGroupMerchantPay(pay);
            	}else{
            		pay.setMerchantCode(merchant.getOutMchntId());
            		pay.setCreateUser(su.getLoginName());
            		pay.setCreateTime(new Date());
            		groupMerchantPayService.saveGroupMerchantPay(pay);
            	}
            }
    ...
    public String showEditeMerchant() {
        ...
        merchant.setGroupCode(groupCode);
       ...
       // 查询开户行
        ...
        bankInfo.setId(null);
        final List<BankInfo> bankInfosList = bankInfoService.queryBankInfoList(bankInfo);
        // 查询门店信息
...
//					 p.setId(pList.get(0).getId());
					...
					if (str.contains(Constant.RESULT_CODE_SUCCESS)){
						...
						//代理商商户
						GroupMerchantPay pay=new GroupMerchantPay();
						pay.setMerchantCode(merchantCode);
						pay.setMerchantUser(userCode);
						groupMerchantPayService.updateByMerchantCode(pay);
	...
	public String getGroupCode(){
		...
		if (userRelationshipList != null & userRelationshipList.size() > 0)
		{
			...
			groupCode=userRelationship.getGroupCode();
		}
		return groupCode;
	}
