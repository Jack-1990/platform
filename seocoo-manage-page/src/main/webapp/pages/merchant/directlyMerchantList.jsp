			...
			<tbody>
				<c:forEach items="${merchantPage}" var="merchant" varStatus="vs">
					...
					<tr <c:if test="${vs.index %2 ==0 }">class="even"</c:if>>
						...
						<td><c:if test="${merchant.flag < 1}">
								<a href="javascript:void(0);" id="inTo" onclick="inToBlance(this)">进件</a>
							</c:if> <c:if test="${merchant.flag >= 1}">
								<a href="javascript:void(0);" id="inTo" onclick="inToBlance(this)">重新进件</a>
								<a href="javascript:void(0);" onclick="showbindingBlance(this)">绑定</a>
							</c:if> 
							<a href="javascript:void(0);" onclick="showEditMerchant(this)">编辑</a> <!-- <a href="javascript:void(0);" class="cancel">打印设置</a> -->
							<input type="hidden" id="merchantId" value="${merchant.id }" />
							<a href="javascript:void(0);" onclick="binMerUser(this)">管理账户</a>

							<%--<c:if test="${merchant.pay == 1}">--%>
								<%--<span>代理商商户</span>--%>
							<%--</c:if> --%>
						</td>
					</tr>
...
<script type="text/javascript">
	...
	if ($("#Pagination").html() == "")
	{
		$("#Pagination").pagination(total,
		{
			...
			'prev_text' : "上一页",
			'next_text' : "下一页",
			'callback' : pageselectCallbackMerc
		});
	}

	function pageselectCallbackMerc(page_index, jq)
	{
		var merchant_Select = $("#merchant_Select").val();
		var merchant_Input = $("#merchant_Input").val();
		directlyMerchantList(page_index,merchant_Select,merchant_Input);
	}
</script>
