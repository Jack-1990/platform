				...
				<c:forEach items="${merchantPage}" var="merchant" varStatus="vs">
					...
					<tr <c:if test="${vs.index %2 ==0 }">class="even"</c:if>>
						...
						<td><c:if test="${merchant.flag < 1}">
								<a href="javascript:void(0);" id="inTo" onclick="inToBlance_s(this)">进件</a>
							</c:if> <c:if test="${merchant.flag >= 1}">
								<a href="javascript:void(0);" id="inTo" onclick="inToBlance_s(this)">重新进件</a>
								<a href="javascript:void(0);" onclick="showbindingBlance_s(this)">绑定</a>
							</c:if> 
							<a href="javascript:void(0);" onclick="showEditMerchant_s(this)">编辑</a> <!-- <a href="javascript:void(0);" class="cancel">打印设置</a> -->
							<input type="hidden" id="merchantId" value="${merchant.id }" />
							<a href="javascript:void(0);" onclick="binMerUser_s(this)">管理账户</a>
							 <span>收款商户</span>
						</td>
					</tr>
